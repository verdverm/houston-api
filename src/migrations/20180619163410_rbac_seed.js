...
const scopedPermissions = {
  user: {
    // Permissions given to a User with an association of the object in which they are interacting with
    user: {
      ...
      service_account:
        "Requestor can manage Service Accounts specifically associated with them"
    },

    group: {
      // Linking Users and Roles (permissions) to access all associated deployments
    ...
    group_user: {
      // Individual Users linked to a Group
      add: "Requestor can add another User to a specific Group",
      remove:
        "Requestor can remove themselves or another User from a specific Group",
      list: "Requestor can view all Users associated with a specific Group",
      manage_permissions:
        "Requestor can add or remove Roles that are associated with a specific Group"
    },

    role: {
      // Grouping of specific permission for the system
    ...
    workspace_user: {
      // Individual Users linked to a Workspace
      add: "Requestor can add another User to a specific Workspace",
      remove:
        "Requestor can remove themselves or another User from a specific Workspace",
      list: "Requestor can view Users associated with a Workspace"
    },

    workspace_service_account: {
      // Individual Service Accounts linked to a Workspace
    ...
    deployment: {
      ...
      resources:
        "Requestor can update details around computing resources allocated by the system for a specific Deployment",
      images:
        "Requestor can update details around the Docker Image used by a specific Deployment",
      external:
        "Requestor has access to view the integrated dashboard associated with a specific Deployment"
    },

    deployment_service_account: {
      // Individual Service Accounts linked to a Deployment
      create: "Create service accounts for a workspace or deployment",
      list: "List services accounts for workspace or deployments",
      view: "View service accounts for workspace or deployment",
      update: "Update service accounts for workspace or deployment",
  ...
  global: {
    ...
    group: {
      // Linking Users, and defining their Roles (permissions), to access all associated deployments
      create: "Requestor can create a new Group",
      list: "Requestor can list all Groups within the system",
      view:
        "Requestor can review information about a specific Group within the system",
      update:
        "Requestor can update information about a specific Group within the system",
    ...
    service_account: {
      // Individual Service Accounts that have access at a system level
      ...
      view:
        "Requestor can review information about a specific Service Account with system-wide access",
      update:
        "Requestor can update the details of a Service Accounts with system-wide access",
      delete: "Requestor can remove a Service Accounts with system-wide access"
    },

    system_setting: {
      // Ability to control specific system-wide configuration details
...
export function up(knex) {
  ...
  return Promise.all(promises).then(() => {
    ...
    for (let key in settings) {
      ...
      groupInserts.push(
        knex(SYSTEM_SETTING_TABLE).insert({
          key: setting.key,
          value: setting.uuid,
          category: "internal",
          is_encrypted: false,
